...
基于 `bilibili-api` + `blivedm` 的直播间弹幕自动响应机器人，支持弹幕关键词回复、点歌、查歌、礼物/SC/上舰感谢、进场欢迎、自动禁言、粉丝排行、数据统计等功能。
...
- **弹幕区**（左侧）：实时显示弹幕、SC、礼物、上舰等消息，按类型着色
- **功能面板**（右侧）：点歌列表、粉丝排行、数据统计等
- **底栏**：人气值实时显示、设置开关（欢迎/感谢/禁言等）
- **菜单**：设置面板可独立开关各功能
