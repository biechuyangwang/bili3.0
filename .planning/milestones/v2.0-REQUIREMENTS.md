...
- [x] **LOG-01**: Each room's log writes to a separate folder: logs/{room_id}/danmaku.log
- [x] **LOG-02**: Log files use same TimedRotatingFileHandler pattern (daily rotation, 30-day retention)
- [x] **LOG-03**: Adding a room creates its log folder; removing a room does not delete existing logs

### Error Recovery

- [x] **ERR-01**: One room disconnecting does not crash or affect other room connections
- [x] **ERR-02**: Failed room connection shows error status on its tab (red dot + error message)
- [x] **ERR-03**: User can retry connecting to a failed room by closing and re-adding it
- [x] **ERR-04**: Application enforces a configurable maximum room limit (default: 5)
